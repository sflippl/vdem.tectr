...
```{r}
load_all()
pdffile <- system.file("extdata", "Country_Year_V-Dem_STATA_v8",
...
```{r}
categories <- str_detect(codebook2, "^\\s{2,3}\\S[^:]+:") & 
...
The pattern is typically "<number>: <response>". However, there is a typo in "v3regendtypems" which we will correct beforehand.

```{r}
tbl_categories[tbl_categories$ind_name == "v3regendtypems", "Responses"][[1]][[1]] %<>%
  str_replace("11.", "11:")
```
...
```{r}
metaframe <- 
  rename(tbl_categories, name = ind_name, fxInfo_name = ind_name_external) %>%
  inner_join(select(tbl_parts, part_num, part_title), by = "part_num") %>% 
  left_join(select(tbl_chapters, chapter_num, chapter_title, part_num), 
             by = c("part_num", "chapter_num")) %>% 
  group_by(part_num, chapter_num, ind_num)
path <- paste0(system.file("inst", "extdata", package = "vdem.tectr"), 
               "/metaframe")
