...
Here, we determine the `fxGeom_class`. The existence of variables with the suffix _codehigh and _codelow indicates a CI; the remaining extrapolations are clear.
...
```{r}
create_assoc_vars <- function(nam) {
  codehigh <- paste0(nam, "_codehigh")
  codelow <- paste0(nam, "_codelow")
  bool_ci <- 
    codehigh %in% names(vdem) & 
    codelow %in% names(vdem)
  if(nam == "year") aes(group = country_name)
  else if(bool_ci) aes_string(lower = codelow, upper = codehigh, 
                              group = "country_name")
  else NULL
}
vdem <-
...
```{r}
metaframe(vdem)$fxInfo_scale %>% unique()
```
...
```{r}
metaframe(vdem)$fxInfo_responses %>% 
...
vdem <- 
  vdem %>% 
  mutate_mf(
    fxGeom_limits = fx_default_fxGeom_limits(vdem), 
    fxGeom_limits = 
      purrr::map(
        seq_len(nrow(metaframe(vdem))), 
        function(i) {
          if(name[i] == "country_name") return(NULL)
...
vdem <- 
  ...
  mutate_mf(
    fxGeom_trans = map2(
      fxGeom_limits, fx_default_fxGeom_trans(vdem), 
      ~ if(!is.null(.x)) return(scales::identity_trans()) else .y
    )
  )
```
...
vote_step <- function(nomination, aes_name, ...) {
  ...
  has_path <-
    nomination %>% 
    nom_layers() %>%
    purrr::map_lgl( ~ inherits(.$geom, "GeomStep")) %>% 
    any()
  2 * has_path
}
...
vdem <- 
  ...
  mutate_mf(
    fxGeom_veto = map(
      fxInfo_scale, 
      ~ if(. == "Nominal" & !(is.na(.))) return(veto_lines) else NULL
    ),
    fxGeom_vote = 
      map(
        seq_len(nrow(metaframe(vdem))), 
        function(i) {
          tmp_mf <- metaframe(vdem)[i, ]
          if(
            tmp_mf$part_num == 3 & 
              (
                (tmp_mf$chapter_num == 1 & tmp_mf$ind_num >= 9 & tmp_mf$ind_num <= 55) | 
                  (tmp_mf$chapter_num == 2 & tmp_mf$ind_num >= 9 & tmp_mf$ind_num <= 15) |
                  (tmp_mf$chapter_num == 13 & tmp_mf$ind_num >= 17 & tmp_mf$ind_num <= 22)
              ) & 
            !is.na(tmp_mf$part_num) & !is.na(tmp_mf$chapter_num) & !is.na(tmp_mf$ind_num)
          ) return(vote_step)
          NULL
        }
      )
...
vdem <-
  vdem %>% 
  mutate_mf(fxGeom_drop = (name == "country_name"))
```
...
qualitative_strong <- scales::brewer_pal(palette = "Set1")
```
    ...
    fxGeom_pal.colour_fill = 
      map(
        ...
        function(i) {
          ...
          if(tmp_mf$part_num == 2 & !is.na(tmp_mf$part_num)) return(bad_good_cont)
          # All osp/ord variables are formulated so that lower values are 
          # less democratic
          if(
            (
              str_detect(tmp_mf$fxInfo_name, "(Original)") | 
              str_detect(tmp_mf$fxInfo_name, "(Relative)")
            ) & !is.na(tmp_mf$fxInfo_name)
          ) return(bad_good_cont)
          if(str_detect(tmp_mf$fxInfo_name, "(Ordinal)") & !is.na(tmp_mf$fxInfo_name)) return(bad_good_disc)
          if(tmp_mf$name == "v3partyid") return(pal_v3partyid)
          if(tmp_mf$fxInfo_scale == "Nominal" & !is.na(tmp_mf$fxInfo_scale)) return(qualitative)
          if(tmp_mf$name == "country_name") return(qualitative_strong)
          if(is(fxGeom(tmp_mf$fxGeom_class[[1]]), "fxGeomContinuous")) return(sequential_cont)
          if(is(fxGeom(tmp_mf$fxGeom_class[[1]]), "fxGeomDiscrete")) return(sequential_disc)
          NULL
        }
      )
  )
...
We will not save the geometry in the same metaframe for reasons of size. However, we do need a metaframe row geometry for when the geometry is added via `vdem_geocode()`. 

```{r}
new_metaframe <-
  metaframe(vdem) %>% 
  bind_rows(
    tibble(name = "geometry", fxInfo_name = "Country Area", 
           fxGeom_class = "Spatial")
  ) %>% 
  as_metaframe()
metaframe(vdem) <- new_metaframe
```
